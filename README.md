...
3. 商品画像用のドライブフォルダを用意します。フォルダ直下に「カテゴリ名（シート名）」ごとのサブフォルダを作成し、その中に商品名（シートA列）と同名の画像ファイルを配置してください。  
...
2. `Code.gs` 冒頭の定数を環境に合わせて設定します。
   ...
   ```
3. 必要に応じて `回答` シートが自動作成されるため、回答保存用のシートを事前に用意する必要はありません。
...
- フロントエンドは1ページ構成です。  
  ...
  3. 任意で自由記入欄に要望を入力。  
- 「送信」ボタンで回答を送信すると、`回答` シートに「タイムスタンプ／メールアドレス／選択商品一覧／自販機メーカー画像ID・名称／自由記入」が追記されます。
